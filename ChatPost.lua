...
-- `/redbook post` can only ever print locally.
--
...
-- and no separate preview form -- `/redbook post` prints the same line, so what you check is what
...
-- Death report:
--   RedBook - died to Khardamu Blood-sworn's Cleave for 15,482 | 01:33 fight
...
local function DeathSegments(session)
	...
	-- "RedBook - " prefix at all (a chat post is obviously from a person).
	segments[table.getn(segments) + 1] = {
