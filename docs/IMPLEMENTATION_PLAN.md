...
**Goal:** `/plugins load RedBook` prints a line and nothing errors.

- `RedBook.plugin` + `RedBook.plugincompendium` (keep `<Version>` in sync in
...
- `Settings.lua` — load/save via
  `Turbine.PluginData.Save(Turbine.DataScope.Character, "RedBook", settings)`,
  plus a `FixColors()` that rebuilds **every** colour key on load.
  Read unknown keys defensively.
- `/redbook` shell command with `help`.
...
- Sanity check before touching UI: `/redbook dump` writing totals to chat. Feed the
  captures in `reference/` through the parser line by line as fixtures and
  compare.

**Done when:** `/redbook dump` after a fight prints totals that match the log.
...
- `/redbook` subcommands: `show`, `hide`, `move`, `reset`, `dump`.
