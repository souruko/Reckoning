...
- `load_test.lua` loads **`Main.lua` itself**, top to bottom, constructing all four windows and
  ...
  `Events.heartbeat:Update()` and runs every `/basil` subcommand.
...
| `resources_test.lua` | That every `"<Package>/Resources/*.tga"` path in the source resolves to a file that exists, under a prefix matching the `.plugin`'s own `<Package>` -- plus the stroke ladder agreeing with `tools/icons/build_icons.py` and every sprite being `STROKE_NATIVE` square. The only file here that reads the tree rather than running it, because a path the client cannot resolve throws nothing: it writes "Failed to load background image" to `Script.log` and draws an empty control. |
...
| `analysis_test.lua` | The whole analysis window: tab order, the block stack's y offsets, table columns fitting their viewport in all four views, KPI cards not colliding, range dragging rescoping every widget, buff charting, resize at both extremes, and (section 18) the post button's invisible quickslot overlay tracking the window through drag, resize, show/hide, re-raise and shutdown. Section 19 holds the rotation pass off while the window is hidden; section 20 switches between two real fights and checks every segment on screen carries the angle it was drawn for -- a *stale* angle is what section 19's "is it rotated at all" check cannot see. |
