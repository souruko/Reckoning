...
for _, cmd in ipairs({ "help", "post", "post death", "options",
                       "show analysis", "hide analysis", "move analysis" }) do
  local okCmd, errCmd = Run(cmd)
  check("/basil " .. cmd, okCmd, okCmd and "" or tostring(errCmd))
end
-- The diagnostic commands (dump, testdeath, buffs, probe) were removed for release. An unknown
-- subcommand must print the "unknown command" line rather than throw.
for _, cmd in ipairs({ "dump", "testdeath", "buffs", "probe", "nonsense" }) do
  local okCmd, errCmd = Run(cmd)
  check("/basil " .. cmd .. " is rejected, not thrown", okCmd, okCmd and "" or tostring(errCmd))
end
