...
function Events.heartbeat:Update()
	...
	-- And the same reason again, for the graph's rotation pass: Analysis:Update stops being called
	-- the moment that window is hidden, so it cannot see itself being closed and cannot arm a fresh
	-- pass when it is opened again. This heartbeat runs either way -- see Analysis:TrackVisibility.
	if analysis ~= nil and analysis.TrackVisibility ~= nil then
		analysis:TrackVisibility()
	end
end
