...
windows (live meter, death cause, post-combat analysis), and the options panel / `/redbook` (alias `/rb`)
...
The version appears in **two** files and must be kept in sync: `RedBook.plugin`
(`<Version>`) and `RedBook.plugincompendium` (`<Version>`), plus `Constants.lua`
