...
-- player's (options window, Self buffs page), not a guess about what counts as a buff.
--
...
-- running game -- a wrong entry here is harmless (it simply never matches). The analysis window's
-- SELF EFFECTS table lists what is actually being tracked, and the options window's Self buffs
-- page is where a player adds their own. That page, not this table, is the real mechanism.
Buffs.Ignore = {
	["Riding"] = true,
	["Mounted"] = true,
	["War-steed"] = true,
...
-- `false` case, un-ignoring a default like "Riding" would silently do nothing and a default entry
-- that turns out to match a real, wanted effect could never be recovered.
