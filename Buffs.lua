...
-- player's (`/redbook buffs ignore <name>`), not a guess about what counts as a buff.
--
...
-- running game -- a wrong entry here is harmless (it simply never matches), and `/redbook buffs`
-- lists what is actually being tracked so a player can add their own with
-- `/redbook buffs ignore <name>`. That command, not this table, is the real mechanism.
Buffs.Ignore = {
	["Riding"] = true,
	["Mounted"] = true,
	["War-steed"] = true,
...
-- `false` case, `/redbook buffs unignore Riding` would silently do nothing and a default entry that
