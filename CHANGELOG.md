...
- The command is **`/basil`** — `/basil options`, `/basil post` and the rest all work exactly as
  they did, just under the new name. `/reck` is gone.
...
- **Charting an effect no longer flattens the plot.** Ticking a self-effect on to the graph added
  its lane but left the lines above it drawn as flat bars until you nudged the range slider or
  picked another fight. The lines are now redrawn properly the moment a lane is added or removed.
- A lane whose icon art fails to load falls back to the effect's initials, as it was always meant
  to, instead of taking the whole plot down with it.

**The leftover development commands are gone.** `/basil dump`, `/basil testdeath`, `/basil buffs`
and `/basil probe` were all built to check the plugin against itself while it was being written,
and none of them showed you anything the windows do not. What is left is `/basil help`, `options`,
`post`, `show`, `hide`, `move` and `reset`.

- Ignoring a self-effect is now done in one place: `/basil options` → **Self buffs**, where the
  ignore list is a set of chips you can add to and click to remove.
- Two options rows that were never wired to anything — **Rows shown** on the Live meter page and
  **Row density** on the Appearance page — no longer appear. Neither ever changed anything; your
  saved values are kept in case they come back.
